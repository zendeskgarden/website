...
Font sizes are used in conjunction with [`lineHeights`](#lineHeights) to
define the basis for Garden’s [type scale](/components/typography#type-scale).
The scale reduces [monospace](/components/typography#font-modifiers)
equivalents by one pixel so that x-height is proportional with the
surrounding system font.
...
Line heights are used in conjunction with [`fontSizes`](#fontSizes) to define
the basis for Garden’s [type scale](/components/typography#type-scale). See
the [`getLineHeight`](/components/utilities#getLineHeight) utility for
obtaining unitless line height values.
...
The RTL theme object value determines whether components will layout
