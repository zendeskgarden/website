---
title: Radio
description: >
  Radio buttons let users choose a single option among two or more mutually exclusive options.
package: '@zendeskgarden/react-forms'
components:
  - forms/src/elements/common/Field.tsx
  - forms/src/elements/common/Fieldset.tsx
  - forms/src/elements/Radio.tsx
subcomponents:
  - forms/src/elements/common/Hint.tsx
  - forms/src/elements/common/Label.tsx
  - forms/src/elements/common/Legend.tsx
  - forms/src/elements/common/Message.tsx
---

import { SEO } from '../../components';

export function Head(props) {
  return <SEO {...props} />;
}

<Usage>
  <Use>

    - For choices or options that can't occur at the same time
    - To indicate that two or more options are mutually exclusive

  </Use>
  <Misuse>

    - If the user can choose more than one option at once, use a [Checkbox](/components/checkbox) instead
    - To select from a long list of options, use [Combobox](/components/combobox) instead

  </Misuse>
</Usage>

## How to use it

### Default

A Radio’s label is part of its touch target.

import Default from '../../examples/forms/radio/Default.tsx';
import DefaultCode from '!!raw-loader!../../examples/forms/radio/Default.tsx';

<CodeExample code={DefaultCode}>
  <Default />
</CodeExample>

### Disabled

A disabled Radio prevents user interaction. It doesn’t appear in the tab order, can’t receive focus, and may not read
aloud by a screenreader.

import Disabled from '../../examples/forms/radio/Disabled.tsx';
import DisabledCode from '!!raw-loader!../../examples/forms/radio/Disabled.tsx';

<CodeExample code={DisabledCode}>
  <Disabled />
</CodeExample>

### Hidden label

Radio labels can be hidden.

import Hidden from '../../examples/forms/radio/Hidden.tsx';
import HiddenCode from '!!raw-loader!../../examples/forms/radio/Hidden.tsx';

<CodeExample code={HiddenCode}>
  <Hidden />
</CodeExample>

### Hint text

Hint text adds additional context to a Radio label.

import Hint from '../../examples/forms/radio/Hint.tsx';
import HintCode from '!!raw-loader!../../examples/forms/radio/Hint.tsx';

<CodeExample code={HintCode}>
  <Hint />
</CodeExample>

### Regular weight label

Radio labels are bold by default, but can be regular weight too.

import RegularWeightLabel from '../../examples/forms/radio/RegularWeightLabel.tsx';
import RegularWeightLabelCode from '!!raw-loader!../../examples/forms/radio/RegularWeightLabel.tsx';

<CodeExample code={RegularWeightLabelCode}>
  <RegularWeightLabel />
</CodeExample>

### Validation

Success, warning, and danger validation messages are shown using the Message component.

import Validation from '../../examples/forms/radio/Validation.tsx';
import ValidationCode from '!!raw-loader!../../examples/forms/radio/Validation.tsx';

<CodeExample code={ValidationCode}>
  <Validation />
</CodeExample>

## Configuration

<Configuration reactPackage={props.data.mdx.package} components={props.data.mdx.components} />

## API

The Radio component follows this structure:

```jsx
<Field>
  <Radio>
    <Field.Label />
    <Field.Hint />
  </Radio>
  <Message />
</Field>
```

### Field

<Component components={props.data.mdx.components} componentName="field" />

A Field provides accessibility attributes to its child [Radio](#radio) field by associating it
with the corresponding [Field.Label](#fieldlabel) and [Field.Hint](#fieldhint).

<PropSheet components={props.data.mdx.components} componentName="field" />

#### Field.Hint

<Component components={props.data.mdx.subcomponents} componentName="field.hint" />

Nest the Hint within the [Radio](#radio) component.

<PropSheet components={props.data.mdx.subcomponents} componentName="field.hint" />

#### Field.Label

<Component components={props.data.mdx.subcomponents} componentName="field.label" />

Nest the Label within the [Radio](#radio) component.

<PropSheet components={props.data.mdx.subcomponents} componentName="field.label" />

#### Field.Message

<Component components={props.data.mdx.subcomponents} componentName="field.message" />

The Message component applies appropriate icon and styles based on the validation provided. Nest
it within a [Field](#field) component.

<PropSheet components={props.data.mdx.subcomponents} componentName="field.message" />

### Fieldset

<Component components={props.data.mdx.components} componentName="fieldset" />

The Fieldset component provides styles for grouping child form inputs. Wrap related [Field](#field)
components and a [Legend](#fieldsetlegend) together within a Fieldset component. See [validation example](#validation) usage.

<PropSheet components={props.data.mdx.components} componentName="fieldset" />

#### Fieldset.Legend

<Component components={props.data.mdx.subcomponents} componentName="fieldset.legend" />

Nest a Legend within a [Fieldset](#fieldset) component.

<PropSheet components={props.data.mdx.subcomponents} componentName="fieldset.legend" />

### Radio

<Component components={props.data.mdx.components} componentName="radio" />

Nest a Radio within a [Field](#field) component.

<PropSheet components={props.data.mdx.components} componentName="radio" />

import { graphql } from 'gatsby';

export const pageQuery = graphql`
  query ($slug: String) {
    ...SidebarPageFragment
  }
`;
