...
Drag and drop interaction is common in workflows where the user wants to change the order or of items.  
...
The grip handle and border define the default Draggable item. The grip icon provides a visual  
affordance to communicate drag-and-drop. Border communicates the boundaries of a draggable item.  
Draggable items with many interactive elements can restrict movement to the grip handle only.

![Draggable item with default, selected, dragged, and disabled state comparison.](figma:patterns-dnd-formatting-draggable.png)

1. **Default** state communicates what is draggable
2. **Selected** state communicates the item selected with a keyboard
3. **Dragged** state communicates the item moving
4. **Disabled** state communicates a non-interactive item
...
Nested elements will make the parent element taller and harder to interact with. Instead of making it transparent,
transform it into a simpler draggable item while dragging.

<BestPractice>
  <Do
    imageSource={
      props.data.dndFormattingDraggablePreviewDoThis.childFile.childImageSharp.gatsbyImageData
    }
  >
    <ul>
  ...
  <Dont
    imageSource={
      props.data.dndFormattingDraggablePreviewNotThis.childFile.childImageSharp.gatsbyImageData
    }
  >
    <ul>
...
Target indicators show where dragged items will appear upon release. Use them in condensed layouts
or when you don't want to transform the list while dragging.

![Two examples showing when vertical and horizontal indicators appear when a draggable item is dragged.](figma:patterns-dnd-formatting-target-indicator-vertical-horizontal.png)
...
Dropzones are locations on the screen where the item can be moved onto. For flat structure lists,
the dropzone is usually the list container. In complex workflows, dropzone might be another list — flat or nested,
or even another item. Dropzones can be communicated in several ways, by using dashed borders, labels or a
task-oriented context of reordering or building something on the screen.
...
Dropzone labels and default states explain to the user what will happen before they start the interaction,
or what is the end result. This is most evident when items are being moved to an empty dropzone, or where
the intention might be to nest or replace the item. Content, together with supporting icons and colors
will make the user confident in the end result.
...
1. Use descriptive labels for empty dropzones
...
See [Draggable tables](https://garden.zendesk.com/components/table#draggable) for reordering rows.

Once the item is grabbed, empty space proportional to the selected item will communicate where the item will
...
- `Up` and `Down` arrows keys change the position order of a selected item within a single container
- `Left` and `Right` arrow keys move the selected item between different containers on the screen.
...
- `Esc` key cancels movement of the selected item and returns it to the original position
...
export const pageQuery = graphql`
  query ($fileAbsolutePath: String) {
    ...SidebarPageFragment
    dndIntro: figmaAsset(name: { eq: "patterns-dnd-intro" }) {
      childFile {
        childImageSharp {
    ...
    dndAnatomy: figmaAsset(name: { eq: "patterns-dnd-anatomy" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 300)
        }
    ...
    dndAnatomyFlow1: figmaAsset(name: { eq: "patterns-dnd-anatomy-flow-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 300)
        }
    ...
    dndAnatomyFlow2: figmaAsset(name: { eq: "patterns-dnd-anatomy-flow-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 300)
        }
    ...
    dndAnatomyFlow3: figmaAsset(name: { eq: "patterns-dnd-anatomy-flow-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 300)
        }
    ...
    dndAnatomyFlow4: figmaAsset(name: { eq: "patterns-dnd-anatomy-flow-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 300)
        }
    ...
    dndFormattingDraggable: figmaAsset(name: { eq: "patterns-dnd-formatting-draggable" }) {
      childFile {
        childImageSharp {
    ...
    dndFormattingDraggablePreviewDoThis: figmaAsset(
    ...
    dndFormattingDraggablePreviewNotThis: figmaAsset(
    ...
    dndFormattingPlaceholderEmpty: figmaAsset(
    ...
    dndFormattingPlaceholderSolid: figmaAsset(
    ...
    dndFormattingTargetIndicator: figmaAsset(
    ...
    dndFormattingPlaceholdersAndDisabled: figmaAsset(
    ...
    dndFormattingTargetIndicatorOrientation: figmaAsset(
    ...
    dndFormattingDropzoneStates: figmaAsset(
    ...
    dndFormattingDropzoneExamples: figmaAsset(
    ...
    dndFormattingDropzoneEmptyPopulated: figmaAsset(
    ...
    dndFormattingDropzoneReplace: figmaAsset(
    ...
    dndFlowsReorder1: figmaAsset(name: { eq: "patterns-dnd-flows-reorder-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsReorder2: figmaAsset(name: { eq: "patterns-dnd-flows-reorder-2" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsReorder3: figmaAsset(name: { eq: "patterns-dnd-flows-reorder-3" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsReorder4: figmaAsset(name: { eq: "patterns-dnd-flows-reorder-4" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMultipleLists1: figmaAsset(name: { eq: "patterns-dnd-flows-multiple-lists-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMultipleLists2: figmaAsset(name: { eq: "patterns-dnd-flows-multiple-lists-2" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMultipleLists3: figmaAsset(name: { eq: "patterns-dnd-flows-multiple-lists-3" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMultipleLists4: figmaAsset(name: { eq: "patterns-dnd-flows-multiple-lists-4" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting1: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-1" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting2: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-2" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting3: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-3" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting4: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-4" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting5: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-5" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsMovingNesting6: figmaAsset(name: { eq: "patterns-dnd-flows-moving-nesting-6" }) {
      childFile {
        childImageSharp {
          gatsbyImageData(height: 400)
        }
    ...
    dndFlowsCollisionAlgorithm1: figmaAsset(
    ...
    dndFlowsCollisionAlgorithm2: figmaAsset(
    ...
    dndFlowsCollisionAlgorithm3: figmaAsset(
    ...
    dndAccessibilityAlternative1: figmaAsset(
